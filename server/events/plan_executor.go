...
func (p *PlanExecutor) RunPlan(ctx *CommandContext, projects []models.Project) CommandResponse {
	cloneDir, err := p.Workspace.Clone(ctx.Log, ctx.BaseRepo, ctx.HeadRepo, ctx.Pull, ctx.Command.Environment)
	if err != nil {
		return CommandResponse{Error: err}
	}
	...
	for _, project := range projects {
		ctx.Log.Info("running plan for project at path %q", project.Path)
		// result := p.plan(ctx, cloneDir, project)
		result := p.planNoLock(ctx, cloneDir, project)
		result.Path = project.Path
		results = append(results, result)
	}
	return CommandResponse{ProjectResults: results}
...
func (p *PlanExecutor) planNoLock(ctx *CommandContext, repoDir string, project models.Project) ProjectResult {
	// preExecute := p.ProjectPreExecute.Execute(ctx, repoDir, project)
	preExecute := p.ProjectPreExecute.ExecuteNoLock(ctx, repoDir, project)
	...
	tfPlanCmd := append(append([]string{"plan", "-detailed-exitcode", "-refresh", "-no-color", "-out", planFile, "-var", userVar}, planExtraArgs...), ctx.Command.Flags...)
	...
	exitCode, output, err := p.Terraform.RunCommandWithVersionCaptureExit(ctx.Log, filepath.Join(repoDir, project.Path), tfPlanCmd, terraformVersion, tfEnv)
	ctx.Log.Info("EXIT CODE: %d", exitCode)
	if err != nil {
...
func (p *PlanExecutor) plan(ctx *CommandContext, repoDir string, project models.Project) ProjectResult {
	...
	return ProjectResult{
		PlanSuccess: &PlanSuccess{
			TerraformOutput: output,
			LockURL:         p.LockURL(preExecute.LockResponse.LockKey),
			// todo: use exit code from plan to determine changes
			Changes: false,
		},
	}
}
